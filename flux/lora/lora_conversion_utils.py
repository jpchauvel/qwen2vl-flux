...
def _maybe_map_sgm_blocks_to_diffusers(
    state_dict, unet_config, delimiter="_", block_slice_pos=5
):
    ...
    input_blocks = {
        layer_id: [
            ...
            if f"input_blocks{delimiter}{layer_id}" in key
        ]
    ...
    middle_blocks = {
        layer_id: [
            key
            for key in state_dict
            if f"middle_block{delimiter}{layer_id}" in key
        ]
...
def _convert_non_diffusers_lora_to_diffusers(
    ...
    dora_present_in_unet = any(
        "dora_scale" in k and "lora_unet_" in k for k in state_dict
    )
    dora_present_in_te = any(
        "dora_scale" in k and ("lora_te_" in k or "lora_te1_" in k)
        for k in state_dict
    )
...
def _convert_text_encoder_lora_key(key, lora_name):
    ...
    if lora_name.startswith(("lora_te_", "lora_te1_")):
        key_to_replace = (
            "lora_te_" if lora_name.startswith("lora_te_") else "lora_te1_"
        )
    else:
