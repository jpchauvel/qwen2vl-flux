...
def parse_args():
    ...
    parser.add_argument(
        "--steps", type=int, default=28, help="Number of inference steps"
    )
    parser.add_argument(
        "--guidance_scale",
        type=float,
...
def main():
    ...
    # Check CUDA and MPS availability if requested
    ...
    print(
        f"Initializing model on {args.device} with features: {required_features}"
    )
    model = FluxModel(
        is_turbo=args.turbo,
        device=args.device,
    ...
    reference_image = (
        load_image(args.reference_image) if args.reference_image else None
    )
